...
def dashboard():
    ...
    for item in benefits:
        ...
        elif target == "차량" and user.get("car") == "yes":
            result.append(item)
            saving += item["saving"]

        elif target == "신혼" and user.get("marriage") == "기혼":
            result.append(item)
            saving += item["saving"]

        elif target == "다자녀" and int(user.get("children", 0)) >= 2:
            result.append(item)
            saving += item["saving"]

        elif target == "사업자" and user.get("business") != "없음":
            result.append(item)
            saving += item["saving"]

    score = min(100, int(saving / 5000))
